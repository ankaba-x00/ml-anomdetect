...
a. Make sure you execute all scripts from $PROJECT_ROOT which is ./ml-anomdetect. 

b. If you executed the file from $PROJECT_ROOT and still get a FileNotFound Error, this could indicates an error in a previous workflow step. Please check your stdout carefully for any errors which you will find denoted as "[ERROR]" in the stdout or manually view the file dependencies at the beginning of each pipeline file under section "PARAMS" and ensure all dependies have been generated successfully.  
...
a. Check the raw json file for this country. If indeed the values are missing, you have to fetch this dataset again. 
